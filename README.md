...
* Mapping the data
  - Source json fields can be mapped to schema fields. Easy convert from one json structure to another.
  - Automap all fields that are exactly matching the schema fields.
* Editing the data
  - Create a schema based on the loaded json fields using the builtin SchemaBuilder or import schema file.
...
- [ ] Add a changelog to the settingspage. It should consist of a header and a RichText label.
...
- [ ] Add metadata with enums in bulk_editor modal. Add correct ui element for cahnging.
...
- [ ] Add an option to each schema field in the SchemaMapper to add default data before loading to table.
- [ ] Add detailed status / debug in the footer
- [ ] Center all the checkboxes in the columns. Should he persistant when resizing column.
